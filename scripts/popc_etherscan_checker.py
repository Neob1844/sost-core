...
def get_balance(config, contract, address, decimals):
    """Get token balance trying Etherscan first, then public RPCs."""
    # Try Etherscan V2
    ...
    # Try public RPCs
    for rpc in config.get("eth_rpc_endpoints", PUBLIC_RPCS):
        ...
        while True:
            for entry in registry:
                ...
                if result["status"] == "CUSTODY_VIOLATION":
                    logging.warning(f"!!! CUSTODY VIOLATION: {result['eth_address']}")
                time.sleep(0.5)
            time.sleep(interval)
