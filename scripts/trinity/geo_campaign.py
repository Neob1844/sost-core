#!/usr/bin/env python3
"""Trinity / Geo Discovery — Campaign Manifest builder v0.1.

Composes a Geo dossier and a Geo Useful Compute plan into one canonical
campaign manifest: ``TRINITY_GEO_CAMPAIGN_<campaign>.json`` + Markdown
sidecar.

The manifest carries:

- A **closed evidence-gap taxonomy** (8 entries) geo-specific.
- A **6-bucket NextAction taxonomy**: ``immediate_local`` /
  ``useful_compute_candidate`` / ``needs_external_data`` /
  ``needs_operator_review`` / ``blocked`` / ``unsafe_or_forbidden``.
- A **forbidden-substring veto** that routes any action whose title or
  description matches a closed list of disallowed phrases to the
  ``unsafe_or_forbidden`` bucket.
- A **ranking key** ``(safety asc, -impact, prereq_count, bucket,
  action_id)`` that pushes unsafe actions to the end.
- Five anchor ``unsafe_or_forbidden`` entries (rewards, publishing,
  on-chain registration without operator, fund moves, consensus
  modification).

Invariants
----------
- DRY-RUN. ``safety_status.dry_run = true``,
  ``ready_to_register = true``, ``registered = false``.
- No network, no RPC, no subprocess, no broadcast.
- Deterministic canonical JSON.
- No host-path leak.
"""

from __future__ import annotations

import argparse
import hashlib
import json
import sys
from pathlib import Path
from typing import Any, Dict, List, Optional, Tuple


_SCHEMA = "trinity-geo-campaign/v0.1"
_DOSSIER_SCHEMA = "trinity-geo-dossier/v0.1"
_PLAN_SCHEMA = "trinity-geo-uc-plan/v0.1"
_TRACK = "geaspirit"
_HOST_PREFIXES = ("/home/", "/opt/", "/Users/", "C:/", "C:\\")

_BUCKET_IMMEDIATE = "immediate_local"
_BUCKET_UC = "useful_compute_candidate"
_BUCKET_EXTERNAL = "needs_external_data"
_BUCKET_REVIEW = "needs_operator_review"
_BUCKET_BLOCKED = "blocked"
_BUCKET_UNSAFE = "unsafe_or_forbidden"

_ALL_BUCKETS = (
    _BUCKET_IMMEDIATE,
    _BUCKET_UC,
    _BUCKET_EXTERNAL,
    _BUCKET_REVIEW,
    _BUCKET_BLOCKED,
    _BUCKET_UNSAFE,
)

_BUCKET_ORDER = {b: i for i, b in enumerate(_ALL_BUCKETS)}
_SAFETY_ORDER = {"safe": 0, "unsafe": 1}
_IMPACT_ORDER = {"high": 0, "medium": 1, "low": 2, "none": 3}


_GAP_TAXONOMY: List[Dict[str, str]] = [
    {
        "gap_id": "gap_no_field_validation",
        "label": "No field validation on file",
        "match_substrings": "field validation,field geological,field geophysics",
    },
    {
        "gap_id": "gap_no_drilling_evidence",
        "label": "No drilling evidence on file",
        "match_substrings": "drilling,drill",
    },
    {
        "gap_id": "gap_no_geophysics",
        "label": "No geophysics survey lines on file",
        "match_substrings": "geophysics,gravity,magnetics,em survey,aem",
    },
    {
        "gap_id": "gap_no_geological_mapping",
        "label": "No detailed geological mapping",
        "match_substrings": "geological mapping,geological survey,mapping 1:250",
    },
    {
        "gap_id": "gap_no_soil_geochemistry",
        "label": "No soil-geochemistry transect",
        "match_substrings": "soil geochemistry,soil-geochemistry",
    },
    {
        "gap_id": "gap_protected_area_legal_unknown",
        "label": "Protected / legally-uncertain area",
        "match_substrings": "protected-area,protected area,legal status,national park",
    },
    {
        "gap_id": "gap_low_data_availability",
        "label": "Low public data availability for region",
        "match_substrings": "data availability,data scarce,no public data",
    },
    {
        "gap_id": "gap_too_close_to_known_demo",
        "label": "Too close to a v0 demo AOI",
        "match_substrings": "near_known_demo_aoi,known demo,demo aoi",
    },
]


_FORBIDDEN_SUBSTRINGS = (
    "activate rewards",
    "publish reward",
    "register on chain",
    "broadcast capsule",
    "move funds",
    "modify consensus",
    "change consensus",
    "auto-broadcast",
    "auto register",
)


_ANCHOR_UNSAFE_ACTIONS: List[Dict[str, Any]] = [
    {
        "action_id": "act_no_activate_rewards",
        "title": "Do not activate Useful Compute rewards",
        "description": (
            "Useful Compute rewards stay dry-run until a separate "
            "consensus / governance procedure ships. The engine never "
            "flips that switch."
        ),
        "matched_substring": "activate rewards",
    },
    {
        "action_id": "act_no_publish_reward_tasks",
        "title": "Do not publish reward-bearing tasks",
        "description": (
            "The public Useful Compute worker / API is unchanged. The "
            "engine never enqueues a paid task there."
        ),
        "matched_substring": "publish reward",
    },
    {
        "action_id": "act_no_register_on_chain_without_operator",
        "title": "Do not register on chain without operator decision",
        "description": (
            "Capsule registration is the operator's manual decision. "
            "The engine prepares a ready-to-register bundle and stops."
        ),
        "matched_substring": "register on chain",
    },
    {
        "action_id": "act_no_move_funds",
        "title": "Do not move funds",
        "description": (
            "The engine touches no wallet, no key, no transaction "
            "broadcast path."
        ),
        "matched_substring": "move funds",
    },
    {
        "action_id": "act_no_modify_consensus",
        "title": "Do not modify consensus",
        "description": (
            "Consensus rules, miner code, node code and the RPC schema "
            "are strictly out of scope."
        ),
        "matched_substring": "modify consensus",
    },
]


def canonical_dumps(obj: Any) -> str:
    return json.dumps(
        obj, sort_keys=True, separators=(",", ":"), ensure_ascii=True,
    )


def _check_no_host_paths(blob: str) -> None:
    leaked = [m for m in _HOST_PREFIXES if m in blob]
    if leaked:
        raise ValueError(
            f"refusing to emit campaign: host-path markers leaked into "
            f"canonical JSON: {leaked}"
        )


def _assign_gaps_for_text(text: str) -> List[str]:
    haystack = text.lower()
    matched: List[str] = []
    for g in _GAP_TAXONOMY:
        for needle in g["match_substrings"].split(","):
            needle = needle.strip().lower()
            if needle and needle in haystack:
                if g["gap_id"] not in matched:
                    matched.append(g["gap_id"])
                break
    return matched


def _forbidden_match(title: str, description: str) -> Optional[str]:
    haystack = (title + " " + description).lower()
    for needle in _FORBIDDEN_SUBSTRINGS:
        if needle in haystack:
            return needle
    return None


def _ranking_key(action: Dict[str, Any]) -> Tuple[int, int, int, int, str]:
    return (
        _SAFETY_ORDER.get(action.get("safety", "safe"), 0),
        _IMPACT_ORDER.get(action.get("impact", "none"), 3),
        len(action.get("prereqs", []) or []),
        _BUCKET_ORDER.get(action.get("bucket", _BUCKET_IMMEDIATE), 0),
        action.get("action_id", ""),
    )


def _propose_actions_for_aoi(
    aoi: Dict[str, Any],
    plan_proposals_by_aoi: Dict[str, List[Dict[str, Any]]],
) -> List[Dict[str, Any]]:
    actions: List[Dict[str, Any]] = []
    aid = aoi.get("aoi_id") or "?"
    region = aoi.get("region") or "unknown"
    decision = aoi.get("decision")

    # Assign gaps from this AOI's evidence_gaps + missing_evidence
    gap_ids: List[str] = []
    text_pool: List[str] = []
    text_pool.extend(aoi.get("evidence_gaps") or [])
    text_pool.extend(aoi.get("missing_evidence") or [])
    for t in text_pool:
        for gid in _assign_gaps_for_text(t):
            if gid not in gap_ids:
                gap_ids.append(gid)

    # 1) UC actions from the compute plan
    uc_props = plan_proposals_by_aoi.get(aid, [])
    impact = "high" if decision == "accept" else (
        "medium" if decision == "hold" else "low"
    )
    for pf in uc_props:
        if pf.get("classification") == "candidate_reward_worthy":
            actions.append({
                "action_id": f"{aid}_uc_{pf['family_id']}",
                "aoi_id": aid,
                "title": (
                    f"Useful-Compute candidate task: "
                    f"{pf['human_label']} for {aid}"
                ),
                "description": pf["rationale"],
                "bucket": _BUCKET_UC,
                "safety": "safe",
                "impact": impact,
                "prereqs": [],
            })

    # 2) External-data actions when gaps point to public data layers
    if "gap_no_geophysics" in gap_ids:
        actions.append({
            "action_id": f"{aid}_external_geophysics",
            "aoi_id": aid,
            "title": (
                f"Source public geophysics layers for {aid} ({region})"
            ),
            "description": (
                "Pull global gravity (WGM2012) and magnetic (EMAG2) "
                "grids over the AOI bbox; verify resolution suits the "
                "AOI size."
            ),
            "bucket": _BUCKET_EXTERNAL,
            "safety": "safe",
            "impact": "medium",
            "prereqs": ["gap_no_geophysics"],
        })
    if "gap_no_geological_mapping" in gap_ids:
        actions.append({
            "action_id": f"{aid}_external_mapping",
            "aoi_id": aid,
            "title": (
                f"Find local 1:250k geological mapping for {aid}"
            ),
            "description": (
                "Check the national geological survey of the AOI's "
                "country for 1:250k or finer mapping sheets covering "
                "the bbox. Open data preferred."
            ),
            "bucket": _BUCKET_EXTERNAL,
            "safety": "safe",
            "impact": "medium",
            "prereqs": ["gap_no_geological_mapping"],
        })

    # 3) Operator-review actions
    if "gap_protected_area_legal_unknown" in gap_ids:
        actions.append({
            "action_id": f"{aid}_review_legal",
            "aoi_id": aid,
            "title": (
                f"Operator review: legal status for {aid} ({region})"
            ),
            "description": (
                "AOI overlaps a protected / legally-uncertain area. "
                "Operator must clear legality before any field "
                "validation or public claim."
            ),
            "bucket": _BUCKET_REVIEW,
            "safety": "safe",
            "impact": "medium",
            "prereqs": ["gap_protected_area_legal_unknown"],
        })

    # 4) Reject AOIs → low-impact archive marker
    if decision == "reject":
        actions.append({
            "action_id": f"{aid}_archive",
            "aoi_id": aid,
            "title": f"Archive {aid} as council-rejected AOI",
            "description": (
                "Rejected by AI Council; keep as calibration anchor in "
                "the regional descriptor cross-check pool."
            ),
            "bucket": _BUCKET_REVIEW,
            "safety": "safe",
            "impact": "low",
            "prereqs": [],
        })

    # 5) Immediate local "log gap" action for everything else
    unaddressed = [
        g for g in gap_ids
        if g not in (
            "gap_no_geophysics",
            "gap_no_geological_mapping",
            "gap_protected_area_legal_unknown",
        )
    ]
    if unaddressed:
        actions.append({
            "action_id": f"{aid}_log_open_gaps",
            "aoi_id": aid,
            "title": f"Log open evidence gaps for {aid}",
            "description": (
                f"Record the open evidence gaps "
                f"{sorted(unaddressed)} in the campaign log."
            ),
            "bucket": _BUCKET_IMMEDIATE,
            "safety": "safe",
            "impact": "low",
            "prereqs": list(sorted(unaddressed)),
        })

    return actions


def _apply_safety_veto(actions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for a in actions:
        match = _forbidden_match(
            a.get("title", ""), a.get("description", "")
        )
        if match:
            ra = dict(a)
            ra["bucket"] = _BUCKET_UNSAFE
            ra["safety"] = "unsafe"
            ra["forbidden_substring_matched"] = match
            out.append(ra)
        else:
            out.append(a)
    return out


def build_campaign(
    *,
    campaign: str,
    generated_at_utc: str,
    dossier_path: Path,
    plan_path: Path,
) -> Dict[str, Any]:
    if not isinstance(campaign, str) or not campaign.strip():
        raise ValueError("campaign must be a non-empty string")
    if not isinstance(generated_at_utc, str) or not generated_at_utc.endswith(
        "+00:00"
    ):
        raise ValueError(
            "generated_at_utc must be ISO-8601 ending in +00:00"
        )
    if not dossier_path.exists():
        raise FileNotFoundError(
            f"geo dossier not found at {dossier_path}"
        )
    if not plan_path.exists():
        raise FileNotFoundError(
            f"geo compute plan not found at {plan_path}"
        )

    dossier = json.loads(dossier_path.read_text(encoding="utf-8"))
    plan = json.loads(plan_path.read_text(encoding="utf-8"))

    if dossier.get("schema") != _DOSSIER_SCHEMA:
        raise ValueError(
            f"dossier {dossier_path.name} schema must be "
            f"{_DOSSIER_SCHEMA!r}"
        )
    if plan.get("schema") != _PLAN_SCHEMA:
        raise ValueError(
            f"plan {plan_path.name} schema must be {_PLAN_SCHEMA!r}"
        )
    if dossier.get("track") != _TRACK or plan.get("track") != _TRACK:
        raise ValueError("dossier or plan declares a non-geaspirit track")

    dossier_sha = hashlib.sha256(dossier_path.read_bytes()).hexdigest()
    plan_sha = hashlib.sha256(plan_path.read_bytes()).hexdigest()

    proposals_by_aoi: Dict[str, List[Dict[str, Any]]] = {}
    for ap in plan.get("aoi_proposals", []):
        proposals_by_aoi[ap["aoi_id"]] = ap.get("proposed_families", [])

    seen_gaps: Dict[str, int] = {}
    per_aoi_gaps: Dict[str, List[str]] = {}
    for a in dossier.get("aois", []):
        aid = a.get("aoi_id") or "?"
        gids: List[str] = []
        text_pool: List[str] = []
        text_pool.extend(a.get("evidence_gaps") or [])
        text_pool.extend(a.get("missing_evidence") or [])
        for t in text_pool:
            for gid in _assign_gaps_for_text(t):
                if gid not in gids:
                    gids.append(gid)
                seen_gaps[gid] = seen_gaps.get(gid, 0) + 1
        per_aoi_gaps[aid] = gids

    raw_actions: List[Dict[str, Any]] = []
    for a in dossier.get("aois", []):
        raw_actions.extend(
            _propose_actions_for_aoi(a, proposals_by_aoi)
        )

    for u in _ANCHOR_UNSAFE_ACTIONS:
        raw_actions.append({
            **u,
            "aoi_id": None,
            "bucket": _BUCKET_UNSAFE,
            "safety": "unsafe",
            "impact": "none",
            "prereqs": [],
            "forbidden_substring_matched": u["matched_substring"],
        })

    veto_actions = _apply_safety_veto(raw_actions)
    ranked = sorted(veto_actions, key=_ranking_key)

    gap_inventory: List[Dict[str, Any]] = []
    for g in _GAP_TAXONOMY:
        gap_inventory.append({
            "gap_id": g["gap_id"],
            "label": g["label"],
            "aoi_count": seen_gaps.get(g["gap_id"], 0),
        })

    by_bucket: Dict[str, int] = {b: 0 for b in _ALL_BUCKETS}
    for a in ranked:
        by_bucket[a.get("bucket", _BUCKET_IMMEDIATE)] += 1

    manifest = {
        "schema": _SCHEMA,
        "campaign": campaign,
        "track": _TRACK,
        "commodity": dossier.get("commodity"),
        "generated_at_utc": generated_at_utc,
        "source": {
            "dossier_basename": dossier_path.name,
            "dossier_sha256": dossier_sha,
            "plan_basename": plan_path.name,
            "plan_sha256": plan_sha,
        },
        "safety_status": {
            "dry_run": True,
            "ready_to_register": True,
            "registered": False,
            "no_rewards_active": True,
            "no_chain_broadcast": True,
            "no_consensus_modification": True,
            "no_public_publication": True,
            "no_wallet_action": True,
        },
        "evidence_gap_inventory": gap_inventory,
        "per_aoi_gap_assignments": [
            {"aoi_id": aid, "gap_ids": per_aoi_gaps[aid]}
            for aid in sorted(per_aoi_gaps)
        ],
        "next_actions": ranked,
        "summary": {
            "aois_total": len(per_aoi_gaps),
            "actions_total": len(ranked),
            "by_bucket": by_bucket,
            "gaps_observed": sum(
                1 for g in gap_inventory if g["aoi_count"] > 0
            ),
        },
    }
    blob = canonical_dumps(manifest)
    _check_no_host_paths(blob)
    return manifest


def render_markdown(m: Dict[str, Any]) -> str:
    lines: List[str] = []
    lines.append(
        f"# Trinity / Geo Discovery — Campaign Manifest "
        f"`{m['campaign']}`"
    )
    lines.append("")
    lines.append(
        "> **DRY-RUN manifest.** Composes the geo dossier and the geo "
        "Useful Compute plan into one campaign with explicit "
        "evidence-gap inventory and 6-bucket next-actions. "
        "``ready_to_register=true`` but ``registered=false``; on-chain "
        "anchoring is a separate operator decision. Remote proxy "
        "evidence only; not a mineral reserve claim."
    )
    lines.append("")
    lines.append(f"- **Schema**: `{m['schema']}`")
    lines.append(f"- **Track**: `{m['track']}`")
    lines.append(f"- **Commodity**: `{m.get('commodity')}`")
    lines.append(f"- **Generated (UTC)**: {m['generated_at_utc']}")
    src = m["source"]
    lines.append("- **Source**:")
    for k in sorted(src):
        lines.append(f"  - `{k}`: `{src[k]}`")
    lines.append("")
    lines.append("## Safety status")
    lines.append("")
    for k in sorted(m["safety_status"]):
        lines.append(f"- `{k}`: `{m['safety_status'][k]}`")
    lines.append("")
    lines.append("## Evidence-gap inventory (closed taxonomy)")
    lines.append("")
    for g in m["evidence_gap_inventory"]:
        lines.append(
            f"- `{g['gap_id']}` &mdash; {g['label']} "
            f"(observed in {g['aoi_count']} AOI"
            f"{'s' if g['aoi_count']!=1 else ''})"
        )
    lines.append("")
    lines.append("## Top 30 next actions (ranked)")
    lines.append("")
    for a in m["next_actions"][:30]:
        lines.append(
            f"- **{a['title']}** &mdash; bucket `{a['bucket']}` "
            f"&mdash; safety `{a['safety']}` &mdash; impact "
            f"`{a['impact']}` &mdash; id `{a['action_id']}`"
        )
        lines.append(f"  - {a['description']}")
    return "\n".join(lines) + "\n"


def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="geo_campaign",
        description=(
            "Build the Trinity / Geo Discovery Campaign Manifest. "
            "Dry-run; never broadcasts, signs or registers."
        ),
    )
    p.add_argument(
        "--campaign", type=str, default="global_phase1",
    )
    p.add_argument(
        "--dossier", type=str, default=None,
    )
    p.add_argument(
        "--plan", type=str, default=None,
    )
    p.add_argument(
        "--generated-at-utc", type=str,
        default="2026-05-10T00:00:00+00:00",
    )
    p.add_argument("--out-json", type=str, default=None)
    p.add_argument("--out-md", type=str, default=None)
    args = p.parse_args(argv)

    dossier_path = Path(
        args.dossier
        or f"TRINITY_GEO_DOSSIER_{args.campaign}.json"
    )
    plan_path = Path(
        args.plan
        or f"TRINITY_GEO_USEFUL_COMPUTE_PLAN_{args.campaign}.json"
    )
    out_json = Path(
        args.out_json
        or f"TRINITY_GEO_CAMPAIGN_{args.campaign}.json"
    )
    out_md = Path(
        args.out_md
        or f"TRINITY_GEO_CAMPAIGN_{args.campaign}.md"
    )

    manifest = build_campaign(
        campaign=args.campaign,
        generated_at_utc=args.generated_at_utc,
        dossier_path=dossier_path,
        plan_path=plan_path,
    )

    out_json.write_text(canonical_dumps(manifest), encoding="utf-8")
    out_md.write_text(render_markdown(manifest), encoding="utf-8")

    s = manifest["summary"]
    print(f"[geo-campaign] wrote {out_json}")
    print(f"[geo-campaign] wrote {out_md}")
    print(
        f"[geo-campaign] actions: total={s['actions_total']}, "
        f"by_bucket={s['by_bucket']}, "
        f"gaps_observed={s['gaps_observed']}"
    )
    return 0


if __name__ == "__main__":
    sys.exit(main())
