...
for M in wrong-winner winner-self payout-plus payout-minus payout-zero reverse-inputs \
         remove-input dup-input foreign-input extra-output dup-jackpot remove-jackpot coinbase-mutate \
         input-index-bump input-txid-flip payout-intmax dup-all-inputs; do
  run_attack "$M"
done
