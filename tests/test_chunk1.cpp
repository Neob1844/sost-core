...
void test_bitsq() {
    ...
    TEST("empty chain -> GENESIS_BITSQ", casert_next_bitsq(empty, 0) == GENESIS_BITSQ);
    ...
    uint32_t d1 = casert_next_bitsq(chain1, 1);
    ...
    // On-schedule chain (600s spacing) -> difficulty unchanged
    {
        ...
        uint32_t d = casert_next_bitsq(on_sched, 50);
    ...
    // Slow blocks (1200s each) -> difficulty drops
    {
        ...
        uint32_t d = casert_next_bitsq(slow, 50);
    ...
    // Difficulty never goes below MIN_BITSQ
    {
        ...
        uint32_t d = casert_next_bitsq(very_slow, 10);
    ...
    // Difficulty never exceeds MAX_BITSQ
    {
        ...
        uint32_t d = casert_next_bitsq(ultra_fast, 500);
...
void test_casert() {
    printf("\n=== CASERT ===\n");
    std::vector<BlockMeta> empty;
    auto dec = casert_compute(empty, 0);
    TEST("empty chain -> warmup", dec.profile_index == 0);
    ...
    auto dec2 = casert_compute(chain, 10);
    TEST("on-target -> L1", dec2.profile_index == 0);
    ...
    auto dec3 = casert_compute(slow, 10);
    TEST("slow chain -> easing or baseline", dec3.profile_index <= 0);
}
