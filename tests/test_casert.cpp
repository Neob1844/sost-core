...
void test_antistall_trigger() {
    ...
    {
        int64_t now = last_block_time + 149999;
        auto dec = casert_mode_from_chain(chain, 500, now);
        TEST("149999s stall: no decay (level=8)", dec.effective_level == 8);
    }

    // At exactly 7200s: decay should NOT yet drop (activation point, 0 decay time)
    {
        int64_t now = last_block_time + 150000;
        auto dec = casert_mode_from_chain(chain, 500, now);
        TEST("150000s stall: decay activates but 0 time => level=8", dec.effective_level == 8);
    }

    // At 7200 + 600 = 7800s: L8+ drops at 600s/level, so 1 level down => 7
    {
        int64_t now = last_block_time + 150600;
        auto dec = casert_mode_from_chain(chain, 500, now);
        TEST("150600s stall: L8 drops to 7", dec.effective_level == 7);
    }
...
void test_decay_cadence() {
    ...
    {
        int64_t now = last_block_time + 210000 + 600;
        auto dec = casert_mode_from_chain(chain, 500, now);
    ...
    {
        int64_t now = last_block_time + 210000 + 1200;
        auto dec = casert_mode_from_chain(chain, 500, now);
    ...
    {
        int64_t now = last_block_time + 210000 + 1800;
        auto dec = casert_mode_from_chain(chain, 500, now);
    ...
    {
        int64_t now = last_block_time + 210000 + 1800 + 1200;
        auto dec = casert_mode_from_chain(chain, 500, now);
    ...
    {
        int64_t now = last_block_time + 210000 + 1800 + 4 * 1200;
        auto dec = casert_mode_from_chain(chain, 500, now);
    ...
    {
        int64_t now = last_block_time + 210000 + 1800 + 4 * 1200 + 2 * 1800;
        auto dec = casert_mode_from_chain(chain, 500, now);
    ...
    {
        int64_t now = last_block_time + 210000 + 100000;
        auto dec = casert_mode_from_chain(chain, 500, now);
...
void test_mining_vs_validation() {
    ...
    // Mining with stall (now_time > 0, 8000s stall): decay applies
    {
        int64_t now = last_block_time + 150600;
        auto dec = casert_mode_from_chain(chain, 500, now);
        // 150600 - 150000 = 600s decay time. L8+ costs 600s => one drop: 8->7
        TEST("mining with 150600s stall: decay applies, level=7", dec.effective_level == 7);
    }
...
void test_resume_behavior() {
    ...
    // During stall: decayed (ahead=200, activation=max(200*600,7200)=120000)
    {
        int64_t now = last_block_stalled + 122800; // 120000+2800
        auto dec = casert_mode_from_chain(chain_stalled, 400, now);
        // 122800 - 120000 = 2800s decay time. L8+: 600s/level => 4 drops in 2400s, 400s remains
...
void test_edge_cases() {
    ...
    // Time boundary: ahead=100 => activation=max(100*600,7200)=60000s
    {
        auto chain = make_ahead_chain(200, 100);
        int64_t last = chain.back().time;

        auto d_before = casert_mode_from_chain(chain, 200, last + 59999);
        TEST("59999s stall (ahead=100): no decay", d_before.effective_level == 5);

        auto d_at = casert_mode_from_chain(chain, 200, last + 60000);
        TEST("60000s stall (ahead=100): decay activates but 0 time", d_at.effective_level == 5);

        // 60000 + 1200 = 61200s. L5: cost=1200 (5>=4, medium) => one drop
        auto d_after = casert_mode_from_chain(chain, 200, last + 60000 + 1200);
        TEST("61200s stall: L5->L4 (medium tier)", d_after.effective_level == 4);
    }
