"""Static safety surface for the governance section of
...
def test_governance_section_present():
    src = _read()
    assert "Governance Review Batch" in src
    assert "Step 6" in src


def test_governance_section_has_no_network_primitives():
    src = _read()
    for token in _FORBIDDEN_NETWORK_PRIMITIVES:
        assert token not in src, (
            f"governance section contains forbidden network "
            f"primitive: {token!r}"
        )


def test_governance_section_does_not_prompt_for_secrets():
    src = _read().lower()
...
def test_governance_section_has_no_sensitive_input_fields():
    src = _read()
...
def test_governance_section_declares_no_automatic_payment():
    src = _read().lower()
    assert "does not pay automatically" in src
    assert "separate" in src and "payment" in src
    assert "governance" in src


def test_governance_section_uses_conservative_policy_label():
    src = _read()
    assert "conservative" in src.lower()


def test_governance_section_carries_cli_command():
    src = _read()
    assert "useful_compute_governance_gate.py" in src
