#!/usr/bin/env bash
# =============================================================================
# run_v15_devnet.sh — real-node V15 lifecycle harness (local DEVNET_FAST chain).
#
...
#   PROVEN (2026-08-04): on DEVNET_FAST a single node+miner crosses Phase2=4, V13=8,
#   the V14 gates, DTD gate=14, V15=18 (T-transition), the DTD every-block regime AND
#   jackpot heights 24/30/36/42/48 in ~140s (height 51), zero rejections. The full
#   V15 lifecycle is reachable in-session here — the SbPoW time-hard blocker is gone.
#
#   NOTE — a live jackpot PAYOUT needs TWO more pieces (next increment):
#     1. Multi-address mining history. With a single miner, the DTD eligibility set is
#        empty at a jackpot height (the only candidate is the current miner, who is
#        excluded), so winner_exists=false and NO jackpot tx is required — the chain
#        advances past the jackpot height without paying. Rotate >=2 miner addresses.
#     2. Miner/template jackpot construction (PART 16): getblocktemplate must inject the
#        canonical TX_TYPE_JACKPOT as the first template tx at a jackpot height (with the
#        block's miner as cur_miner for winner-exclusion). Not yet wired.
#
# Usage:
#   tests/run_v15_devnet.sh [--blocks N] [--target-height H] [--threads T]
...
V15=18; FIRST_J=24; PHASE2=4; DTD_GATE=14
...
if [[ -f "$BUILD_DIR/CMakeCache.txt" ]]; then
  grep -q '^SOST_DEVNET_FORKS:BOOL=ON' "$BUILD_DIR/CMakeCache.txt" \
...
PROFILE="$(rpc getinfo | jfield profile)"
[[ "$PROFILE" == "dev" ]] && ok "node reports dev profile (DEVNET_FAST identity)" || bad "node is NOT dev (getinfo.profile=$PROFILE) — refusing"
[[ "$PROFILE" == "dev" ]] || die "wrong network"
...
# From Phase2 (DEV height 4) the SbPoW requires the miner's signing key, so pass the
# wallet + key label (label "default" is what `newwallet` creates), not just --address.
"$MINER" --profile dev --rpc "127.0.0.1:$RPC_PORT" --address "$ADDR" \
  --wallet "$WORK/w.json" --mining-key-label default \
