...
// With the gate at INT64_MAX (sentinel OFF) the validator never accepts these
...
// Atomic Swap HTLC LOCK output (OTC-1). Payload = HTLC_LOCK_PAYLOAD_LEN (80):
// [hashlock(32) | refund_height(8 LE) | claim_pkh(20) | refund_pkh(20)].
// Gated by atomic_swap_htlc_active_at(); with the gate at INT64_MAX the
// validator rejects this output type as inactive (R11). Pre-activation chain
// replay is bit-identical because this type has never been mined.
constexpr uint8_t OUT_HTLC_LOCK     = 0x12;
// Atomic Swap HTLC CLAIM witness output (OTC-1). Carries the 32-byte preimage
// in payload[0..31] (HTLC_CLAIM_WITNESS_PAYLOAD_LEN). Only valid inside a
// TX_TYPE_HTLC_CLAIM transaction; reveals the preimage that unlocks the
// corresponding HTLC_LOCK utxo so the counterparty's watcher can claim the
// mirror HTLC. Gated by atomic_swap_htlc_active_at().
...
// -----------------------------------------------------------------------------
// HTLC_LOCK payload helpers (OUT_HTLC_LOCK = 0x12) — OTC-1
//
...
//
// Readers tolerate short payloads by returning a zero-filled value so the
// structural rule R17 in src/tx_validation.cpp produces the right error code
// without segfaulting on malformed inputs.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// HTLC_CLAIM_WITNESS payload helpers (OUT_HTLC_CLAIM_WITNESS = 0x13) — OTC-1
// Payload (HTLC_CLAIM_WITNESS_PAYLOAD_LEN = 32 bytes): [0..31] preimage;
// sha256(preimage) must equal the LOCK's hashlock.
// -----------------------------------------------------------------------------
