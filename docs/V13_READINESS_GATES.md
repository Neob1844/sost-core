...
|---|---|---|
| `casert_all_profiles_e7_h35` | `effective_profile_ceiling_at(height)` returns 35 for `height >= V13_HEIGHT`, OR a `CASERT_V13_PROFILE_CEILING` constant lifts the ceiling. | **NOT WIRED** — only `CASERT_HARD_PROFILE_CEILING = 10` and historical `CASERT_CEILING_H13_HEIGHT = 5750` found. Add the helper before V13 cut. |
