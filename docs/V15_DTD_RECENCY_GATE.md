...
| DTD Jackpot block | **20,000 blocks** (~19 weeks) | broad — returns the vault to ~the whole network, not a recent few |
...
## Implementation
- `include/sost/params.h`: `DTD_RECENCY_WINDOW = 5000`, `DTD_JACKPOT_RECENCY_WINDOW = 20000`,
  `dtd_recency_window_at(height)` → 0 for `height < V15_HEIGHT`, else 20000 on jackpot heights / 5000 otherwise.
...
- `tests/test_dtd_recency.cpp` (target `test-dtd-recency`): 13/13 PASS — helper values, pre-V15 no-op,
  normal 5,000 exclusion, jackpot 20,000 exclusion.
...
## Release binaries (SbPoW=ON, TESTNET=OFF, DEVNET=OFF, Release)
