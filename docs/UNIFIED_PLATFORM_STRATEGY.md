# SOST GeoForge: Unified Materials Discovery + Remote Sensing Platform
...
**What nobody else offers:** GeoForge is the first system that closes the loop between computational materials science (graph neural networks predicting material properties from crystal structure) and orbital remote sensing (detecting mineral spectral signatures from space). Every other player in mineral exploration does one or the other. Nobody combines GNN-predicted spectral signatures with automated satellite search, and nobody anchors discoveries to an immutable blockchain ledger.
...
**GeoForge** — "Geo" for geoscience and remote sensing, "Forge" for computational materials creation. The name signals the fusion of two domains.
...
The gap exists because computational materials science and remote sensing geoscience are entirely separate academic disciplines with different conferences, journals, and toolchains. GeoForge bridges them.
...
- Spectral reflectance specifically (the VNIR/SWIR signature measured by satellites) has not been directly predicted by GNN from crystal structure. This is the gap GeoForge would fill.
...
| **GeoForge (SOST)** | Pre-revenue | GNN materials discovery + remote sensing + blockchain. | No geophysical datasets (yet). Small team. Unproven market traction. |
...
*Document prepared for SOST GeoForge strategic planning. All cost estimates, timelines, and probability assessments reflect honest evaluation as of 2026-03-20. This is a living document — update after each phase milestone.*
