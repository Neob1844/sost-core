...
- **OTC-2 — wallet + automation** ✅ DONE — local, non-custodial wallet + watcher + order board, built and tested with the consensus gate left untouched (`INT64_MAX`). No mainnet activation, no BTC signing, no EVM deploy.
  - **CLI/wallet builders** (`src/sost-cli.cpp`): `createhtlclock`, `claimhtlc`, `refundhtlc`, `decodehtlc`, `gethtlcstatus`. The four builders are **offline** (run before wallet load, need no keys), call the OTC-1 handlers locally and print the *unsigned* raw-tx hex / decoded JSON. They never sign, never broadcast, never touch BTC/EVM. While the gate is OFF every builder cleanly returns the disabled message — a deliberate mainnet no-op. `gethtlcstatus` forwards read-only to the node RPC (node-side dispatch is an OTC-3 step).
  - **Order board** (`atomic_swap_orderbook.{h,cpp}`): pure, off-chain maker/taker `Offer` (pair, amounts, hashlock, T1/T2, status, no custody). `ValidateOffer` enforces **timeout ordering** (responder refund opens FIRST, initiator LAST, with a configurable `safety_margin_min_blocks` ≥ 6 gap → mis-ordered/equal/too-close rejected with `TIMEOUT_ORDER_INVALID`) and **structural sanity** (one leg must be SOST, non-zero hashlock, non-zero amounts).
  - **Issuer-freeze honesty**: `AssetHasIssuerFreeze` flags USDT/USDC/PAXG/XAUT (issuer can freeze a leg → atomicity not guaranteed) and surfaces an `ISSUER_FREEZE_RISK` warning on such offers; BTC/ETH/BNB/SOST carry no asset-level freeze.
  - **Watcher / auto-pilot** (`atomic_swap_watcher.{h,cpp}`): pure decision engine over caller-supplied chain facts. `DecideWatchAction` → `Claim` (Claimant, preimage known, before timeout), `Refund` (Refunder, timeout open), `Done` (lock spent/gone), else `Wait`. `IngestRevealedPreimage` accepts a counterparty preimage only on real `sha256(preimage)==hashlock`. `Serialize/ParseWatchlist` persist the watchlist as deterministic text so the wallet **resumes after restart**. The watcher never signs, never broadcasts, never holds funds.
  - **Tests**: `test-atomic-swap-orderbook` + `test-atomic-swap-watcher` cover asset classification, issuer warnings, every `ValidateOffer` reject path, auto-claim/auto-refund timing, preimage match/mismatch, and serialize/watchlist round-trips (resume). **Full ctest 90/90** green (88 pre-existing + 2 new) → no regression, gates and mainnet bytes untouched.
- **OTC-2.5 — node read-only RPC for the watcher** ✅ DONE — wired the live chain-state source the OTC-2 watcher needs, with the consensus gate left untouched (`INT64_MAX`). No mainnet activation, no BTC signing, no EVM deploy, no mutation.
  - **`gethtlcstatus <lock_txid> <lock_vout>`** (`src/sost-node.cpp`): resolves one lock against the live UTXO set + tx index and returns `{status, current_height, is_htlc_lock, spent, amount, hashlock, refund_height, claim_pkh, refund_pkh, revealed_preimage?}`. Status ∈ `locked` (unspent, before timeout) / `expired` (unspent, refund window open) / `claimed` (spent by a `TX_TYPE_HTLC_CLAIM`) / `refunded` (spent by a `TX_TYPE_HTLC_REFUND`) / `unknown`. When a lock was **claimed**, the handler resolves the spending tx and surfaces the **`revealed_preimage`** from its `OUT_HTLC_CLAIM_WITNESS` — the exact value the counterparty wallet ingests (`IngestRevealedPreimage`) to unlock the paired leg.
  - **`listhtlclocks`** (`src/sost-node.cpp`): scans the UTXO set for open `OUT_HTLC_LOCK` outputs (lock discovery), each with the same fields + `locked`/`expired` status.
  - **Pure / read-only**: both handlers only read; they never sign, broadcast, mutate, or add a consensus rule. **Reading chain state is gate-independent** — and because the HTLC gate is OFF on mainnet, consensus rejects every HTLC tx so no `OUT_HTLC_LOCK` can enter the UTXO set; there both RPCs report `unknown` / `[]` (a natural no-op). They become useful on a node where the gate is active (testnet/regtest soak).
  - **Decision core**: `ClassifyHtlcStatus(...)` in `atomic_swap_helpers.{h,cpp}` is the pure, gate-independent classifier; the node handler is a thin I/O adapter (UTXO lookup → if spent, a bounded tx-index-anchored scan for the spender + preimage → classify). Spent-resolution uses a forward scan (no reverse index exists); bounded by chain length and never reached on mainnet.
  - **CLI**: `sost-cli gethtlcstatus <txid> <vout>` and `sost-cli listhtlclocks` forward read-only to the node and print the JSON body.
  - **Tests**: `test-atomic-swap-status` covers every `ClassifyHtlcStatus` branch (unknown/locked/expired/claimed/refunded + spent-but-unresolved), the status name vocabulary, gate-independence, and the **node-status → watcher-action pipeline** (a `gethtlcstatus` result drives `DecideWatchAction` to Wait/Claim/Refund/Done and a revealed preimage is ingested on real sha256 match). **Full ctest 91/91** green (90 + 1 new) → no regression, gates and mainnet bytes untouched.
- **OTC-3a — BTC signing (libwally), regtest-only, OFF by default** ✅ DONE — completed the libwally integration so the BTC HTLC signing backend actually builds and runs, gated behind `-DSOST_BTC_HTLC_SIGNING=ON`. No mainnet, no EVM, no broadcast; consensus gate untouched.
  - **CMake integration** (`CMakeLists.txt`): when the flag is ON, the vendored `vendor/libwally-core` (release_1.5.3) is built in isolation via `ExternalProject` and linked into `sost-core`, with `SOST_BTC_HTLC_SIGNING_HAS_LIBWALLY=1` defined. libwally's bundled secp256k1 is kept isolated from SOST's system `secp256k1` (consensus SbPoW), so the two never collide. The flag also idempotently restores libwally's ceremony-disabled CMake build files. **The default OFF build links no libwally and is byte-identical.**
  - **Backend now real (ON)**: BIP-143 segwit-v0 sighash, Low-R/Low-S ECDSA (libwally `EC_FLAG_GRIND_R`), P2WSH witness assembly (claim `[sig, preimage, 0x01, script]` / refund `[sig, <empty>, script]`), `EncodeP2WSHAddress`, `SignBtcHtlcClaim`, `SignBtcHtlcRefund`. `BuildBtcHtlcRedeemScript` was already real. `SignBtcHtlcLockFunding` stays a stub (funding done via `bitcoin-cli` on regtest).
  - **OFF default unchanged**: every signing function returns `ok=false` (`disabled_result()`); the signing test runs 45 disabled-path assertions. ON runs 80 (incl. the BIP-143 native-P2WSH known-answer vector).
  - **Tests**: `test-atomic-swap-btc-{signing,script,test-vectors}`. **Full ctest 91/91 in BOTH configs** — OFF (default, no libwally) and ON (real backend linked into node/cli/miner/all tests, no duplicate-symbol/link issues).
  - **Docs**: `docs/V15_OTC_BTC_REGTEST_GUIDE.md` — enabling the flag, vendoring libwally, and a full SOST↔BTC regtest cycle (redeemscript → P2WSH fund → claim/refund → preimage extraction → SOST watcher consumes the revealed preimage via OTC-2.5 `gethtlcstatus`).
- **OTC-3b — EVM leg (contract + tests + testnet guide)** ✅ DONE — the EVM counterparty contract is reviewed, fully tested, and documented for testnet, with no mainnet deploy and the SOST gate untouched.
  - **Contract** (`contracts/atomic-swap/src/AtomicSwapHTLC.sol`): minimal dependency-free HTLC. `lockNative` (ETH/BNB) / `lockERC20` (USDT/USDC/PAXG/XAUT/any) / `claim` (preimage, before T2) / `refund` (after T2). `sha256(preimage)` hashlock identical to the SOST validator (R21) and BTC redeem script; absolute `block.number` timeout (T2 < SOST T1 by the wallet margin). State machine `NONE→LOCKED→{CLAIMED|REFUNDED}`, `nonReentrant` + checks-effects-interactions, `receive()`/`fallback()` revert.
  - **Properties confirmed** (source review + `test_noOwnerFunctionsExist_runtime`): **no owner, no admin, no upgrade/`delegatecall`, no pause, no emergency drain** — funds move only via `claim`/`refund` to the recorded `claimer`/`refunder`. Forced ETH (selfdestruct/EIP-6780) is orphaned and never affects accounted swaps.
  - **Tests** (`test/AtomicSwapHTLC.t.sol` + adversarial `test/mocks/MockERC20.sol`): **52 passed, 0 failed** under `forge test`. Covers native+ERC-20 happy paths, wrong preimage, refund-too-early, claim-after-refund, refund-after-claim, double-claim/refund, reentrancy (malicious receiver + malicious token), weird ERC-20 (no-return legacy-USDT rejected at lock; fee-on-transfer documented unsupported), forced-ETH EIP-6780, balance-conservation (drains to exactly 0), and fuzz over preimage + the `refundTime` boundary.
  - **Issuer-freeze**: USDT/USDC/PAXG/XAUT can be frozen by their issuer → EVM leg may become uncollectible mid-swap while the SOST leg still settles. Flagged off-chain by the OTC-2 orderboard (`AssetHasIssuerFreeze` → `ISSUER_FREEZE_RISK`); BTC/ETH/BNB/SOST have no asset-level freeze. Documented in the guide for the UI to surface.
  - **Docs**: `docs/V15_OTC_EVM_TESTNET_GUIDE.md` (build/test, Anvil + Sepolia/BNB-testnet deploy, lock/claim/refund via `cast`, preimage extraction from the `Claimed` event → SOST watcher via OTC-2.5 `gethtlcstatus`). forge-std is gitignored (lean repo); installed with one `forge install`.
- **OTC-4 — end-to-end session coordinator** ✅ DONE — ties the three legs into one orchestrated, non-custodial flow. Pure decision + persistence layer; never signs/broadcasts/holds-a-key, no central order book, gate-agnostic.
  - **Module** (`atomic_swap_session.{h,cpp}`): composes orderbook (`ValidateOffer`), the core coordinator discipline, and the watcher (`IngestRevealedPreimage`). 13-phase machine `Created→Offered→Accepted→SostLocked→CounterpartyLocked→ClaimSeen→Claimed→Completed`, with `RefundReady→Refunded`, `Expired`, `Failed`, `RecoveryNeeded`. `CreateSession` (validates offer + timeout ordering + issuer-freeze; initiator's secret checked against the hashlock), `Ingest`/`IngestPreimage` (phase transitions; preimage accepted only on real `sha256` match), `DecideNextStep` (role-aware next safe action + timeout-margin warnings), `Serialize/ParseSession` (resume; secret flagged + redactable, hashlock re-verified on load; never any private key).
  - **Operator tool** (`tools/otc_swap_coordinator.cpp` → `otc-coordinator`): `create`/`inspect`/`observe`/`next` over a local session file — the operator submits txs with the OTC-1/3a/3b builders and feeds back verified observations; the tool only decides + persists.
  - **Tests** (`test-atomic-swap-session`): full initiator + responder happy paths, refund flow, resume round-trip (with/without secret), and adversarial — wrong preimage, mis-ordered timeouts rejected at creation, responder-locks-too-late, claim-near-timeout warning, out-of-order/duplicate observations, refund-after-preimage-public refused, corrupted/tampered session file rejected, issuer-freeze enforced for USDT/USDC/PAXG/XAUT (and absent for BTC/ETH/BNB), secret discipline at creation. **Full ctest green** (all atomic-swap suites + the rest).
  - **Docs**: `docs/V15_OTC_E2E_SWAP_GUIDE.md` (phase machine, tool↔`otc` command mapping, exact SOST↔BTC-regtest / SOST↔ETH-Anvil / SOST↔ERC20-mock flows, a GO/NO-GO checklist, automated-vs-manual split, issuer-freeze).
