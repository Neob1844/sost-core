...
```
c2b06b91eb9da9f4736cd514df3f7e7616a8961f46b17442d7c171c2f345ac2c  sost-node-sec1   (NEW revised node)
...
| 3 | Clean build of all targets; full suite, no unjustified exclusions | **PASS (Release)** — clean build → node `c2b06b91`; ctest **115/115** with only the 4 `btc-*` tests excluded (need a live bitcoind regtest — justified). |
...
| 6 | Reproducible node hash from the revision tag | **PASS** — `c2b06b91…` reproduces from source commit 260ffd02 (tag `v16.3.0-sec1`). |
...
- **(A) Ship sec1 as-is** (node `c2b06b91`, miner/cli unchanged), and fix the base64 UB across all
...
- **(B) Fold the base64 hardening into sec1** — fixes node + cli + miner, makes the full Debug/ASan
  ...
  `c2b06b91`) and widens sec1 beyond the fork-store/SIGPIPE scope.
