...
**IBD mining gate — SECURITY REVIEW DONE:** v1 (peer-announced-height) was itself a remote mining-DoS (measured 6/20 denial by one lying peer) → **REDESIGNED v2** (`fix/ibd-mining-gate` `7ad9aba0`): gate on OWN validated tip height vs LAST_HARD_CHECKPOINT (attacker-independent, Sybil/eclipse-proof) + miner honors the refusal. Deterministic tests **5/5** (liar/colluding→mines; genesis+honest→recovers; genesis+only-liars→refused-no-genesis-chain; solo→mines). ctest 119/119, no consensus change/deadlock/stall/bypass. **66 historic exceptions assessed = no trust problem** (≤5038, exact-hash-anchored, no PoW bypass, no forward surface). Severity: defense-in-depth (network already rejects stale-tip blocks). Node 550ba580/miner 5a29cad4 — node+miner candidate, dev only, NOT sec2, NOT deployed.
**PHASE 3 — integrated candidate DONE:** `integration/sec2-p2p-ibd` = sec2 + P2P convergence/sync + ibd-gate v2 (node `c0c21da0`/miner `5a29cad4`/cli `489f4374`). Merge: 1 conflict (orphan-store: combined sec1 quota + P2P dedup guard), ibd-gate 0 conflicts. Validated ON THE INTEGRATED BINARY: ctest 119/119, cASERT bit-exact 0 div, reorg 9/0, jackpot_v2 9/0, convergence shallow+genesis + under 120-peer attack (RSS 11MB, 0 crash, 0 unfair-ban), IBD gate 5/5, RPC culprits handled. headers-first EXCLUDED (pending ConvergenceX header audit). TESTING: real-mainnet sync/benchmark + integrated TSan (need mainnet chain/dedicated run). Dev branch only; changes node+miner (NOT sec2, NOT deployed). See `INTEGRATION_CANDIDATE.md`.
**PHASE 3 CLOSURE (3 pending tests + audits) DONE:** 2A real mainnet sync on integrated binary genesis→5587, 5/5 hashes match live chain, ~23 blk/s post-checkpoint (~18min full, extrapolated NOT claimed); 2B 66/66 exception hashes match real mainnet blocks (no substitution); 2C TSan on integrated binary (mining+P2P+reorg+RPC concurrent) 0 races. ConvergenceX header audit: PoW needs full transcript, NOT header-verifiable → headers-first stays OUT, chain-selection = highest-VERIFIED-work. IBD residual documented (DoS-immune+genesis-safe; min-chainwork floor = release-time tightening; no consensus change). See SECURITY_CLOSURE_PHASE3.md. Remaining TESTING: full-tip sync run, disk-full (privileged tmpfs).
**PHASE 3 CLOSURE FINAL (all 3 tests done on integrated binary):** 2A **FULL mainnet sync genesis→tip(28077) in 25m06s**, tip hash + 5 checkpoints match public RPC, emission #25000 correct (18.2min NOT reproduced → real ~25min, correctness verified). 2B 66/66 hashes match live chain + negative-case (tampered block at exception height rejected) + positive (live-sync applies exceptions w/ block_id confirm); doc fixed (replay table →5410, param →5038). 2C TSan integrated 0 races. Disk-full: fail-safe (real chain.json byte-intact, restart recovers). V16 transition validated via jackpot_v2 9/0 (scaled devnet = same code). ConvergenceX: work needs full transcript → headers-first stays OUT. See SECURITY_CLOSURE_PHASE3.md @ integration/sec2-p2p-ibd.
