...
block 30,000 without understanding why.
...
{
  "commitment_id": "…",
  "activation_height": 25000,        // POPC_V15_ACTIVATION_HEIGHT (mainnet)
  "eligibility_height": 30000,       // DTD_POPC_ELIGIBILITY_HEIGHT (mainnet)
  ...
  "carrier_note": "Register alone leaves the PoPC PENDING. Broadcast BOTH (Register then Activate) to be ACTIVE / DTD-eligible from 30000; create AND maintain."
}
```
...
Status values:
- `not_required_yet` — before block 25,000 carriers are rejected by consensus, so none are produced.
...
mainnet until block 25,000 (after PR #25).

## Miner steps (from block 25,000)
...
5. **Eligible from block 30,000** if your PoPC is active and in good standing.
...
confirmed → active for DTD eligibility (not eligible yet / eligible from 30,000).
