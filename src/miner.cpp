...
{
    MineResult mr{}; mr.found = false; mr.attempts = 0;
    ...
    if (powDiffQ != expected) {
        mr.error = "bad-difficulty"; return mr;
    }
    // Scratchpad
    ...
    auto hc = build_header_core(prev_hash, merkle_root, (uint32_t)timestamp, powDiffQ);
    ...
    for (uint32_t nonce = 0; nonce <= max_nonce; ++nonce) {
        ...
        if (res.is_stable && pow_meets_target(res.commit, powDiffQ)) {
            ...
            mr.elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1-t0).count();
            mr.found = true;
            Block& b = mr.block;
            b.height = h;
            b.prev_hash = prev_hash; b.merkle_root = merkle_root;
            b.timestamp = timestamp; b.powDiffQ = powDiffQ;
            b.nonce = nonce; b.extra_nonce = 0;
            b.commit = res.commit;
...
int mine_chain(
    int32_t num_blocks, Profile prof, bool sim_time,
    uint32_t max_nonce, std::function<void(const Block&)> on_block)
{
    ...
    while (num_blocks == 0 || mined < num_blocks) {
        uint32_t powDiffQ = asert_next_difficulty(chain, (int64_t)chain.size());
        uint32_t extra = 0;
        bool found = false;
        while (!found) {
            auto mr = mine_block(chain, prev, mrkl, ts, powDiffQ, max_nonce, prof);
            if (mr.found) {
                found = true;
                ...
                // Verify
                auto vr = verify_block(mr.block, chain, ts, prof, true);
                if (!vr.ok) {
