    ...
    const ConsensusParams& params)
{
    ...
    int32_t n = params.cx_n;
    if(params.verbose) printf("[CX-VERIFY] Phase 1: sanity (x_bytes_len=%zu, n*4=%d, cp_leaves=%zu)\n",
            x_bytes_len, n*4, checkpoint_leaves.size());
    // Phase 1: Sanity
    if ((int32_t)x_bytes_len != n * 4) { if(params.verbose) printf("[CX-VERIFY] Phase 1 FAILED: x_bytes_len=%zu != n*4=%d\n", x_bytes_len, n*4); return false; }
    if (checkpoint_leaves.empty()) { if(params.verbose) printf("[CX-VERIFY] Phase 1 FAILED: checkpoint_leaves empty\n"); return false; }

    // Phase 2: Checkpoint merkle root
    if(params.verbose) printf("[CX-VERIFY] Phase 2: checkpoint merkle (%zu leaves)\n", checkpoint_leaves.size());
    Bytes32 cp_root = merkle_root_16(checkpoint_leaves);
    if (cp_root != checkpoints_root) { if(params.verbose) printf("[CX-VERIFY] Phase 2 FAILED: checkpoints_root mismatch expected=%s got=%s\n", hex(checkpoints_root).c_str(), hex(cp_root).c_str()); return false; }

    // Phase 3: Segment proofs → segments_root
    if(params.verbose) printf("[CX-VERIFY] Phase 3: segment merkle proofs (%zu proofs)\n", segment_proofs.size());
    for (size_t spi = 0; spi < segment_proofs.size(); ++spi) {
        ...
        if (!verify_merkle_proof(lh, sp.merkle_path, sp.leaf.segment_index, segments_root)) {
            if(params.verbose) printf("[CX-VERIFY] Phase 3 FAILED: segment proof %zu invalid (seg_idx=%u, path_len=%zu, leaf=%s, root=%s)\n",
    ...
    // Phase 4: Seed
    if(params.verbose) printf("[CX-VERIFY] Phase 4: seed recompute (nonce=%u extra=%u)\n", nonce, extra_nonce);
    Bytes32 prev_hash;
    std::memcpy(prev_hash.data(), header_core, 32);
    ...
    if(params.verbose) printf("[CX-VERIFY] Phase 5: commit binding (profile_index=%d)\n", params.stab_profile_index);
    std::vector<uint8_t> cbuf;
    append_magic(cbuf); append(cbuf, "COMMIT", 6);
    ...
    if (computed_commit != commit) {
        if(params.verbose) printf("[CX-VERIFY] Phase 5 FAILED: commit mismatch expected=%s got=%s (cbuf_len=%zu metric=%llu)\n",
    ...
    if(params.verbose) printf("[CX-VERIFY] Phase 6: challenge derivation (rounds=%d seg_len=%d)\n", params.cx_rounds, CX_SEGMENT_LEN);
    ...
    if ((int32_t)segment_proofs.size() != actual_chal) {
        if(params.verbose) printf("[CX-VERIFY] Phase 6 FAILED: segment_proofs.size()=%zu != actual_chal=%d (nseg=%d)\n",
                segment_proofs.size(), actual_chal, nseg);
        return false;
    }
    for (int i = 0; i < actual_chal; ++i) {
        if (segment_proofs[i].leaf.segment_index != expected_seg_idx[i]) {
            if(params.verbose) printf("[CX-VERIFY] Phase 6 FAILED: seg_proof[%d].segment_index=%u != expected=%u\n",
    ...
    if(params.verbose) printf("[CX-VERIFY] Phase 7: boundary coherence\n");
    for (size_t spi = 0; spi < segment_proofs.size(); ++spi) {
        const auto& sp = segment_proofs[spi];
        if (sp.leaf.round_start > sp.leaf.round_end) { if(params.verbose) printf("[CX-VERIFY] Phase 7 FAILED: seg %zu start %u > end %u\n", spi, sp.leaf.round_start, sp.leaf.round_end); return false; }
        if (sp.leaf.round_end > (uint32_t)params.cx_rounds) { if(params.verbose) printf("[CX-VERIFY] Phase 7 FAILED: seg %zu end %u > rounds %d\n", spi, sp.leaf.round_end, params.cx_rounds); return false; }
    }

    // Phase 8: Round witness verification
    if(params.verbose) printf("[CX-VERIFY] Phase 8: round witnesses (%zu total, %d chal segs, %d steps each)\n",
    ...
    for (int seg_i = 0; seg_i < actual_chal && seg_i < (int)expected_round_idx.size(); ++seg_i) {
        for (int step_j = 0; step_j < CX_CHAL_STEPS && step_j < (int)expected_round_idx[seg_i].size(); ++step_j) {
            if (rw_idx >= round_witnesses.size()) { if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: rw_idx=%zu >= rw.size=%zu\n", rw_idx, round_witnesses.size()); return false; }
            const auto& rw = round_witnesses[rw_idx++];
            uint32_t r = expected_round_idx[seg_i][step_j];
            if (rw.round_index != r) { if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: rw[%zu].round=%u != expected=%u (seg=%d step=%d)\n", rw_idx-1, rw.round_index, r, seg_i, step_j); return false; }
            ...
            if (rw.scratch_indices[0] != exp_idx0 || rw.scratch_indices[1] != exp_idx1 ||
                ...
                if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: scratch_indices mismatch at round %u (got [%u,%u,%u,%u] expected [%u,%u,%u,%u])\n",
            ...
            for (int vi = 0; vi < 4; ++vi) {
                ...
                if (rw.scratch_values[vi] != expected_val) {
                    if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: scratch_value[%d] mismatch at round %u idx=%u (got %d expected %d, byte_off=%u blk=%u within=%u)\n",
            ...
            if (rw.program_output != exp_prog) {
                if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: program_output mismatch at round %u (got %llu expected %llu)\n",
            ...
            for (int i = 0; i < n; ++i) {
                if (x_exp[i] != rw.x_after[i]) {
                    if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: x_after[%d] mismatch at round %u (got %d expected %d)\n", i, r, rw.x_after[i], x_exp[i]);
                    return false;
                }
            }
            ...
            if (exp_state != rw.state_after) {
                if(params.verbose) printf("[CX-VERIFY] Phase 8 FAILED: state_after mismatch at round %u (got=%s exp=%s)\n",
                        r, hex(rw.state_after).substr(0,16).c_str(), hex(exp_state).substr(0,16).c_str());
                return false;
            }
            if(params.verbose) printf("[CX-VERIFY] Phase 8: round %u OK (seg=%d step=%d)\n", r, seg_i, step_j);
        }
    }

    // Phase 9: Stability basin
    if(params.verbose) printf("[CX-VERIFY] Phase 9: stability basin (scale=%d k=%d margin=%d steps=%d)\n",
            params.stab_scale, params.stab_k, params.stab_margin, params.stab_steps);
    int32_t x[32];
    ...
    if (!verify_stability_basin(x, prob, n, lr_shift, stctx,
        ...
        if(params.verbose) printf("[CX-VERIFY] Phase 9 FAILED: stability basin test failed (metric_out=%llu)\n", (unsigned long long)metric_out);
        return false;
    }

    // Phase 10: Metric
    if(params.verbose) printf("[CX-VERIFY] Phase 10: metric check (got=%llu expected=%llu)\n", (unsigned long long)metric_out, (unsigned long long)stability_metric);
    if (metric_out != stability_metric) {
        if(params.verbose) printf("[CX-VERIFY] Phase 10 FAILED: metric mismatch got=%llu expected=%llu\n", (unsigned long long)metric_out, (unsigned long long)stability_metric);
        return false;
    }

    if(params.verbose) printf("[CX-VERIFY] ALL PHASES PASSED\n");
    return true;
}
