...
static bool process_block(const std::string& block_json, bool reorg_connect) {
    ...
    if (!extends_tip) {
        ...
        if (!parent_known) {
            ...
            std::lock_guard<std::mutex> lk(g_block_index_mu);
            // [P2P dedup guard] Already parked as an orphan? A resend carries no new information; the
            // dedup carve-out above deliberately lets resends reach here — without this guard a peer
            // resending ONE orphan would insert duplicate g_orphans_by_prev entries (multimap) and
            // exhaust the pool. The legitimate cascade is unaffected (process_orphans_for_parent erases
            // the orphan from g_block_index before re-running it).
            if (g_block_index.count(bid)) { fflush(stdout); return false; }
            const size_t obytes = block_json.size();
            // [sec1 fork-store hardening] Global count cap AND per-origin quota (stops one IP/subnet
            // from filling the orphan pool via reconnection). Non-consensus.
