...
**Current state (Phase 26 — terrain depth pilot, GEE terrain infrastructure operational):**
...
- Phase 26: 8 depth sources audited, 3 GEE-accessible used (SRTM, SAR, terrain derivatives). 4 zones piloted with terrain+SAR depth features. Peru depth-only AUC 0.907 (+0.057 vs S2 baseline — terrain features add real value). Zambia depth-only 0.642 (+0.002, neutral). Kalgoorlie 0.774, Chuquicamata 0.769 (no baseline comparison). Canonical 22.8/40 UNCHANGED — terrain features are surface proxies, not subsurface. True depth unlock needs GA gravity, AEM, Earth MRI (all blocked). GEE terrain infrastructure now operational at all 4 zones.
