...
| popc | test-popc | tests/test_popc.cpp |
| popc-tx | test-popc-tx | tests/test_popc_tx.cpp |
| escrow | test-escrow | tests/test_escrow.cpp |
| proposals | test-proposals | tests/test_proposals.cpp |
| dynamic-rewards | test-dynamic-rewards | tests/test_dynamic_rewards.cpp |
| gold-vault | test-gold-vault | tests/test_gold_vault.cpp |

Tests use a simple `TEST(name, condition)` macro — no external framework. 28/28 CTest targets pass.
...
**Current state (v3.2.0 — Phase XVIII):**
- 76,193 materials corpus (JARVIS-DFT + AFLOW), 70+ API endpoints
