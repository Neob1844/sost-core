...
| 8 | static analysis + security review (no critical/high) | 🟡 | Adversarial CONSENSUS audit done (docs/v15/SECURITY_AUDIT_REPORT.md): 0 exploitable, 2 defense-in-depth — B-1 FIXED (value-conservation backstop, payout re-validated), B-2 deferred to post-fork hardening. GCC `-fanalyzer` CLEAN on ALL V15 code (sost-node 1 pre-existing unrelated warning, sost-miner 0, jackpot/params/lottery headers 0 — docs/v15/STATIC_ANALYSIS_REPORT.md). Only cppcheck/clang-tidy cross-check pending (not installed, no sudo → 2nd box) |
| 9 | pre-V15 compatibility | 🟡 | Code-level: every V15 rule is strictly height-gated at 25000 (P9 boundary test proves NEW behaves pre-V15 below the fork). Deployed binary = **v0.3.2** (confirmed via getinfo). The real-OLD-chain load test + OLD↔NEW cross-accept need the deployed binary/chain snapshot on the 2nd box — see REMAINING_GATES.md |
| 10 | exact-activation @25000 | ✅ | `tests/test_v15_activation_boundary.cpp` (ctest `v15-activation-boundary`), MAINNET profile, 15/15: V15_HEIGHT=25000 (not DEV 18/testnet 12500), 24999 pre / 25000 first V15, jackpot@25290, no off-by-one — docs/v15/ACTIVATION_BOUNDARY_REPORT.md |
