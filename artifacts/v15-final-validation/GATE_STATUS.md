...
| 8 | static analysis + security review (no critical/high) | 🟡 | Adversarial CONSENSUS audit done (docs/v15/SECURITY_AUDIT_REPORT.md): 0 exploitable, 2 defense-in-depth — B-1 FIXED (value-conservation backstop, payout re-validated), B-2 deferred to post-fork hardening. GCC `-fanalyzer` on sost-node.cpp CLEAN (1 pre-existing unrelated `-Wunused-result`). cppcheck/clang-tidy + `-fanalyzer` on sost-miner/headers pending (tools/2nd box) |
